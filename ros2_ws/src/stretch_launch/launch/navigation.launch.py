...
def generate_launch_description():
    ...
    stretch_navigation_path = get_package_share_directory("stretch_launch")
    navigation_bringup_path = get_package_share_directory("nav2_bringup")

    teleop_type_param = DeclareLaunchArgument("teleop_type", default_value="joystick", description="how to teleop ('keyboard', 'joystick' or 'none')")

    use_sim_time_param = DeclareLaunchArgument("use_sim_time", default_value="false", description="Use simulation/Gazebo clock")

    autostart_param = DeclareLaunchArgument("autostart", default_value="true", description="Whether to autostart lifecycle nodes on launch")
    ...
    use_navigation_param = DeclareLaunchArgument("use_navigation", default_value="True", description="Whether to use navigation or not")
    ...
    rviz_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([navigation_bringup_path, "/launch/rviz_launch.py"]), condition=IfCondition(LaunchConfiguration("use_rviz"))
    )
