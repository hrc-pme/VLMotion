...
def generate_launch_description():
    stretch_core_path = get_package_share_path("stretch_launch")

    declare_broadcast_odom_tf_arg = DeclareLaunchArgument(
        "broadcast_odom_tf", default_value="True", choices=["True", "False"], description="Whether to broadcast the odom TF"
    )
    ...
    robot_description_content = launch_ros.parameter_descriptions.ParameterValue(
        Command(["xacro ", str(get_package_share_path("stretch_description") / "urdf" / "stretch.urdf")]), value_type=str
    )
    ...
    stretch_driver = Node(
        ...
        emulate_tty=True,
        output="screen",
        remappings=[("cmd_vel", "/stretch/cmd_vel"), ("joint_states", "/stretch/joint_states")],
