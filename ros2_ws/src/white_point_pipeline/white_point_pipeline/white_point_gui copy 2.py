...
class ServerProcess:
    ...
    def start_model_worker(self, host="0.0.0.0", controller_url="http://10.0.0.1:11000",
                           ...
                           model_path="PME033541/vla2", load_4bit=True):
        """啟動 Model Worker"""
        cmd = [
...
class MainWindow(QMainWindow):
    ...
    def send_to_llm(self, text: str, image: PILImage.Image):
        ...
        try:
            ...
            # 決定對話模板（基於模型名稱）
            model_name = "vla2"  # 使用註冊在 Controller 的模型名稱
    ...
    def handle_llm_response(self, response: str):
        ...
        if vectors_2d and self.current_image is not None:
            ...
            try:
                ...
                bytes_per_line = 3 * w_img
                q_image = QImage(rgb_image.data, w_img, h_img, bytes_per_line, QImage.Format_RGB888)
                pixmap = QPixmap.fromImage(q_image)
                ...
                print(f"Failed to display image: {e}", file=sys.stderr)
                import traceback
                traceback.print_exc()
...
def main(args=None):
    ...
    parser.add_argument("--model-path", type=str, default="PME033541/vla2",
